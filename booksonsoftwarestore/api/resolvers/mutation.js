...
module.exports = {
  createBook: async (parent, args, { dataSources, user }, info) => {
    if (!user) {
      return null;
    }

    const book = await dataSources.bookDataSource.createBook(
      args.book
    );
    return book;
  },
  ...
  toggleFavoriteBook: async (parent, args, context, info) => {
    if (context.user) {
      const user = await context.dataSources.userDataSource.toggleFavoriteBook(
        args.bookId,
        context.user.sub
      );
      return user;
